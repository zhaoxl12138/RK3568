# 从零到 Python MVP 学习路线——完整历史版

> 归档日期：2026-07-13。当前阶段导航见 [[02-从零到Python MVP学习路线]]；归档规则见 [[00-归档说明]]。

这份文档按“我从头开始学”的顺序写。
它是当前唯一执行主线。

如果你不知道今天先看什么、先跑什么，就只看这一份。

## 使用规则

每一阶段先看对应的验收标准，再执行命令或阅读长篇实验记录：[[AI Camera分阶段验收标准|07-专项笔记 / 系统 / 分阶段验收标准]]。

```text
从上往下走，一次只过当前阶段。
不要在 01、02、03 三份主线之间来回跳。
```
...
3. 再跑命令或脚本。
4. 看到现象后，回到本阶段补概念。
5. 最后把结果同步到专项笔记和实验产物索引。

## 当前事实

```text
Python MVP 已经跑通。
现在要做的是从阶段0重新复盘，把每一步背后的原理补齐。
```

```text
当前出厂 Buildroot 4.19 系统上，已经跑通的是官方 YOLOv5 RKNN Python MVP。
...
- Camera 数据从摄像头传感器进来。
- AI 推理发生在 YOLOv5 RKNN / NPU 这一层。
- 结果最后显示到 MIPI 屏，并通过 HLS 输出到 Windows。
...
打开后可以按节点理解 Camera 到 HLS 的完整数据流。
如果你更想补“Camera 到 `/dev/video0` 是怎么一步步接起来的”，优先看 `Phase0 Camera 配置全流程`。
...
- [[03-Camera-OpenCV-RKNN汇合路线-历史版|03-Camera-OpenCV-RKNN汇合路线]]
- [[官方AI例程运行记录]]
